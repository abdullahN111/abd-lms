...
const CourseStructure = ({ data }: iAppProps) => {
  ...
  function SortableItem({ children, id, className, data }: SortableItemsProp) {
    const {
      attributes,
      listeners,
      setNodeRef,
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const activeType =
      (active.data.current?.type as "chapter" | "lesson") ?? "lesson";
    const overType =
      (over.data.current?.type as "chapter" | "lesson") ?? "lesson";
              ...
              >
                {(listeners) => (
                  <Card>
                    ...
                    >
                      <div className="flex flex-items justify-between p-3 border-b border-border">
